...
const GalleryImage = (props: Props) => {
  const { loadImage, src, alt, transforms, attr } = props;
  ...
  return (
    <div
      className={`w-[${transforms.width}px] h-[${transforms.height}px] overflow-hidden relative group`}
    >
      <Image
        {...(!check && loadImageProp)}
        src={src}
        ...
        className="transition-all duration-[450ms] ease-in-out group-hover:scale-125 group-hover:contrast-50 group-hover:cursor-pointer"
        style={{
          width: transforms.width,
          ...(!transforms.autoHeight && { height: transforms.height }),
          // @ts-ignore
          objectFit: transforms.fit,
        }}
      />

      <div className="transition-all duration-150 ease-in invisible group-hover:visible absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
        <p className=" text-2xl font-extrabold drop-shadow-lg">
          {transforms.width} x {transforms.height}
        </p>

        <div
          dangerouslySetInnerHTML={{ __html: attr }}
          className="text-center font-bold attr"
        />
      </div>
    </div>
  );
